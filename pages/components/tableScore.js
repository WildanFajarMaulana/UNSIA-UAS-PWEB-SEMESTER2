...
const TableScore = ({ data }) => {
  ...
  const numberedData = sortedData && addNumbering(sortedData);
    ...
    <table className="w-full text-center border-collapse">
      <thead>
        <tr>
          ...
          <th className="border-b border-gray-500 py-4">Quiz</th>
      ...
      <tbody>
        {numberedData?.map((item) => (
          <tr key={item.id}>
            ...
            <td className="border-b border-gray-500 py-4">{item.quizTitle}</td>
